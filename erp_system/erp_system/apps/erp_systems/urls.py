...
from erp_systems.views.user_view import RegisterUserView, UserView, UpdatePassWordView

urlpatterns = [
    re_path(r'^user/login/$', obtain_jwt_token),  # JWT签发和认证视图
    re_path(r'^user/register/$', RegisterUserView.as_view()),  # 用户注册视图
    re_path(r'^user/update_password/(?P<pk>\d+)/$', UpdatePassWordView.as_view()),  # 用户修改密码视图
